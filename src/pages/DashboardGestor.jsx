...
export default function DashboardGestor() {
    ...
    // Recarrega quando o setor global muda
    useEffect(() => {
        const aoMudarSetor = () => { setLoading(true); fetchParcelas(); };
    ...
    return (
        <div className="flex flex-col max-w-6xl w-full mx-auto p-6 lg:p-10 space-y-8">
            {/* TÍTULO DO SETOR ATIVO */}
            <div className="flex items-center gap-2.5">
                <span className={`w-2.5 h-2.5 rounded-full ${getSetorAtivo() === 'ESPORTE' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                <h2 style={heading} className="text-xl text-stone-800">
                    {getSetorAtivo() === 'ESPORTE' ? 'Departamento de Esporte' : 'Departamento de Cultura'}
                </h2>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h2 style={heading} className="text-2xl text-stone-900">
                        Olá, {userName}!
                    </h2>
                ...
                <div className="bg-cream-50 px-4 py-2 rounded-xl border border-cream-200 flex items-center gap-3">
                    <CalendarDays className="w-5 h-5 text-brand-700" />
                    <div>
                        <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-0.5">Analisando Período:</p>
                        <select
                            className="bg-transparent text-stone-800 text-sm font-semibold outline-none cursor-pointer"
            ...
            {loading ? (
                <div className="flex justify-center py-20 text-stone-400"><Clock3 className="w-8 h-8 animate-pulse" /></div>
            ) : (
                <>
                    {/* CARDS DINÂMICOS */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="bg-white p-5 rounded-2xl border border-cream-200 shadow-sm flex flex-col">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-xs font-bold text-stone-500 uppercase tracking-wider">Equipe Ativa</h3>
                                <div className="p-2 bg-brand-50 rounded-xl"><Users className="w-4 h-4 text-brand-700" /></div>
                            </div>
                            <p style={heading} className="text-3xl text-stone-900">{resumo?.totalInstrutores || 0}</p>
                            <p className="text-xs text-stone-500 mt-2">Instrutores cadastrados</p>
                        </div>
                        ...
                        <div className="bg-white p-5 rounded-2xl border border-cream-200 shadow-sm flex flex-col">
                            ...
                            <p style={heading} className="text-3xl text-stone-900">{resumo?.prestacoesRecebidas || 0}</p>
                            <p className="text-xs text-stone-500 mt-2">Prestações validadas</p>
                        </div>

                        <div className="bg-white p-5 rounded-2xl border border-cream-200 shadow-sm flex flex-col">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-xs font-bold text-brand-700 uppercase tracking-wider">Saúde da Parcela</h3>
                                <div className="p-2 bg-brand-50 rounded-xl"><TrendingUp className="w-4 h-4 text-brand-700" /></div>
                            </div>
                            <p style={heading} className="text-3xl text-stone-900">{resumo?.saudeParcela || 0}%</p>
                            <p className="text-xs text-stone-500 mt-2">Do orçamento comprometido</p>
                        </div>
                    </div>

                    {/* LISTA DE PENDENTES */}
                    <div className="bg-white rounded-2xl border border-cream-200 shadow-sm overflow-hidden">
                        <div className={`px-6 py-4 border-b border-cream-200 flex items-center gap-2 ${resumo?.instrutoresPendentes > 0 ? 'bg-amber-50' : 'bg-emerald-50'}`}>
                                    ...
                                    {resumo?.listaPendentes.map((instrutor) => (
                                        <li key={instrutor.id} className="px-6 py-4 flex items-center justify-between hover:bg-cream-50 transition-colors">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="text-sm font-semibold text-stone-900">{instrutor.nome}</p>
